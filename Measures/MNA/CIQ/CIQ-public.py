...
#        print(edge[0])
    f.close()
    return G
G_hu=read_network('Rtax21159.txt')
G_mo=read_network('Rtax7227.txt')
G_rt=read_network('Rtax10090.txt')
...
file=open("alignmentResule.txt.ali","r")
cluster_num=len(file.readlines())#簇的数量
d=defaultdict(list)#字典
c_time=0#1.计数cluster行数；2.用作字典key值
try:
    with open('alignmentResule.txt.ali','r') as dict_file:#读alignment文件
...
#    i_cluster=linecache.getline('09_10.txt',i+1)#簇的行数，从1开始计数
    for data in i_cluster:
        if Node_exist(data,f1):
            i_hu.append(data)
        elif Node_exist(data,f2):
            i_mo.append(data)
        elif Node_exist(data,f3):
            i_rt.append(data)
...
#        j_cluster_data=j_cluster.strip('\n').split(' ')
        ...
        for datas in j_cluster:
            if Node_exist(datas,f1):
                j_hu.append(datas)
            if Node_exist(datas,f2):
                j_mo.append(datas)
            if Node_exist(datas,f3):
                j_rt.append(datas)
