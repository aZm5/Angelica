...
for each in sorted(s):
    ...
    f1 = open(f"./Bitscore_sim_by_tab/{sp1[0:2]}-{sp2[0:2]}.sim","w")# "\t"
    f2 = open(f"./Bitscore_sim_by_white/{sp1[0:2]}-{sp2[0:2]}.sim", "w")  # " "
    f3 = open(f"./E&B_sim_by_tab/{sp1[0:2]}-{sp2[0:2]}.sim", "w")  # "\t"
    f4 = open(f"./E&B_Bitscore_sim_by_white/{sp1[0:2]}-{sp2[0:2]}.sim", "w")  # " "
    ...
    #    print(line)i
        ...
        if p1 in dFirst.keys() and p2 in dSecond.keys() and p1+p2 not in Saved:
            ...
            #     print('!')
            f1.write(f"{dFirst[p1]}\t{dSecond[p2]}\t{bit_score}\n")
            f2.write(f"{dFirst[p1]} {dSecond[p2]} {bit_score}\n")
            f3.write(f"{dFirst[p1]}\t{dSecond[p2]}\t{evalue}\t{bit_score}\n")
            f4.write(f"{dFirst[p1]} {dSecond[p2]} {evalue} {bit_score}\n")
